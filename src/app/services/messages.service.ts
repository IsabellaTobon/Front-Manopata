...
getChatHistory(userId1: number, userId2: number): Observable<any[]> {
  ...
  return this.http.get<any[]>(`${this.apiUrl}/history`, { headers, params }).pipe(
    tap(() => {
      this.notificationsService.showNotification('Historial de mensajes cargado con éxito.', 'success');
    }),
    catchError((error) => {
      this.notificationsService.showNotification('Error al cargar el historial de mensajes.', 'error');
      throw error;
    })
  );
}
